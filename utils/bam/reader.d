...
struct BamBlobReader {
  BgzfStream stream;
  BamHeader header;
  Nullable!RawReadBlob peekbuf; // points to current read
  ...
  Nullable!RawReadBlob fetch() {
    if (!peekbuf.isNull) {
      auto readbuf = peekbuf;
      peekbuf = Nullable!RawReadBlob();
      return readbuf;
    }
    asserte(!empty); // should have been checked for
    immutable block_size = stream.read!int();
    ...
    peekbuf = RawReadBlob(refid,pos,stream.read(data));
    return peekbuf;
  }

...
void printUsage() {
    ...
    stderr.writeln("               write sorted reads in a cBAM file");
}
...
class Sorter {
    ...
    private void mergeSortedChunks(alias comparator)() {
        ...
        if(!write_cbam){
            ...
            import std.file;
            std.file.write(output_filename, "");
            writeln("OPENING FILE...");
            auto fn = std.stdio.File(output_filename, "w");
            writeln("OPENED FILE...");
            BamHeader bamHeader = BamHeader(header, bam.reference_sequences);
            writeln("INIT FILEWRITER...");
            FileWriter fw = new FileWriter(fn, bamHeader);
            writeln("INITED FILEWRITER...");
            RawReadBlob[] recordBuf;
            recordBuf.length = BATCH_SIZE;
            int i = 0;
            writeln("WRITING CBAM...");
            foreach (read; nWayUnion!comparator(alignmentranges)){
                recordBuf[i] = RawReadBlob(read.ref_id, read.position, read._chunk);
                ++i;
                if(i == BATCH_SIZE){
                    fw.writeRowGroup(recordBuf, i);
                    i = 0;
                }
            }
            writeln("WROTE ALMOST ALL CBAM...");
            if(i != 0) fw.writeRowGroup(recordBuf, i);
            writeln("WROTE ALL CBAM...");
            fw.close();
        }
        if (show_progress)
            bar.finish();
...
int sort_main(string[] args) {
    ...
    try {
        ...
        if (sorter.output_filename is null) {
            if(write_cbam){
                sorter.output_filename = setExtension(args[1], "sorted.cbam");
            }
            else{
